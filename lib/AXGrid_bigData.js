...
AXGrid = Class.create(AXJ, {
    ...
    printList: function () {
        ...
        if (cfg.viewMode == "grid") {
	        if(cfg.height == "auto"){
		        //TODO : this.virtualScroll 속성정의 없으면 null
    ...
    contentScrollResize: function (resetLeft) {
        ...
        this.contentScrollResize_timer = setTimeout(function () {
            ...
            if (bodyHeight < scrollHeight && cfg.height != "auto") {
                ...
                // TODO : scrollYHandleHeight 최소 사이즈 예외 처리 최소 높이 = 10
            ...
            if (scrollWidth > bodyWidth && cfg.xscroll) {
                ...
                // TODO : scrollXHandleWidth 최소 사이즈 예외 처리 최소 높이 = 10
    ...
    contentScrollScrollSync: function (pos) {
        ...
        if (pos.left != undefined) {
            ...
            var T = (this.contentScrollYAttr.scrollHeight - this.contentScrollYAttr.bodyHeight) * ( (pos.top) / (this.contentScrollYAttr.scrollTrackYHeight - this.contentScrollYAttr.scrollYHandleHeight) );
    ...
    contentScrollScrollMove: function (event) {
        ...
        if (handleName == "scrollYHandle") {
            ...
            if ((handleTop + this.contentScrollAttrs.handleHeight) > this.contentScrollAttrs.trackHeight+1) {
    ...
    contentScrollTouchMove: function (e) {
        // TODO : 현재 터치무브가 스크롤 핸들 이동량으로 처리되나. 컨텐츠 이동량으로 처리 되도록 변경
        var cfg = this.config;
        var event = window.event || e;
        if (this.contentScrollTouchMoved) {
	        ...
	        if (scrollTouchAttr.th < scrollTouchAttr.h) {
	...
	bigDataSyncApply: function(){
		...
		if(VS.scrollTop != (scrollContentScrollTop = this.scrollContent.position().top)){
			...
			var newStartIndex = (scrollContentScrollTop.abs() / VS.itemTrHeight).ceil() - 5;
			...
			this.cachedDom.thpadding.css({ height: (newStartIndex-1) * VS.itemTrHeight }); // 상단패딩증가
