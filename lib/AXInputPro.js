...
var AXInputConverterPro = Class.create(AXJ, {
	...
	*/
	...
	bindTagSelector: function(objID, objSeq){
		...
		po = [];
		po.push('<div id="'+ cfg.targetID + '_AX_' + objID + '_AX_tagContainer" class="tag-selector-tagcontainer" style="float:left;overflow;">');
		po.push('</div>');
	...
	bindTagSelector_onkeydown: function(e, objID, objSeq){
		var _this = this, cfg = this.config,
			...
			anchorWidth, anchorHeight, styles;
	...
	bindTagSelector_setOptions: function(objID, objSeq, kword){
		...
		for(var i=0,l=obj.config.options.length;i<l;i++){
			...
			po.push("<a " + obj.config.href + " id=\"" + cfg.targetID + "_AX_" + objID + "_AX_" + i + "_AX_option\" class=\"bindSelectorNodes\">" + O[cfg.reserveKeys.optionText] + descStr + "</a>");
		}
		if (po.length == 0) {
	...
	bindTagSelector_addItem: function(objID, objSeq, optionIndex){
		var _this = this, cfg = this.config,
			obj = this.objects[objSeq], po, addOption, pass_add;
		...
		if(pass_add){
			po = [];
			po.push('<span class="tag-selector-tagitem" data-option-value="' + addOption[cfg.reserveKeys.optionValue] + '">');
				po.push( addOption[cfg.reserveKeys.optionText] );
				po.push('<span class="tag-selector-tagitem-remove"></span>');
			po.push('</span>');
