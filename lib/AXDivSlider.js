	...
	init: function(){
		...
		if(cfg.autoSlide){
			...
			this.target.on("mouseover", function(){
				clearInterval(_this.autoSlideInterval);
			});
			this.target.on("mouseleave", function(){
				if(_this.autoSlideInterval) clearInterval(_this.autoSlideInterval);
				_this.autoSlideInterval = setInterval(autoSlideFn, cfg.autoSlideDelayTime);
			});
