...
```
	...
	/**
	 * @method AXTab.removeTab
	 * @param objID {String} - 탭 대상 ID
	 * @param options {Number|String} [last tab index] - 탭 인덱스(Number) or optionValue(String)
	 * @description 탭을 제거 합니다.
	 * @returns {AXTab}
	 * @example
```js
$("#myTab01").removeTab(2);
$("#myTab01").removeTab("optionValue");
```
	 */
	removeTab: function(objID, tabIndex) {
		var cfg    = this.config;
		var objSeq = axdom("#" + objID).data("objSeq");
		var obj    = this.objects[objSeq];
		...
		// obj.config.options
		obj.config.options.splice(tabIndex, 1);
		
		// obj.config, on css update
		if(obj.config.selectedIndex === tabIndex){
			var selectedIndex = tabIndex - 1;
			obj.config.selectedIndex = selectedIndex;
			obj.config.value = obj.config.options[selectedIndex].optionValue;
			
			tabs.eq(selectedIndex).addClass("on");
...
axdom.fn.removeTab = function(tabIndex) {
	axdom.each(this, function () {
		AXTab.removeTab(this.id, tabIndex);
		return this;
	});
};