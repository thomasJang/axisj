...
var AXGridTree = Class.create(AXGrid, {
	...
	setWBS: function(WBS){
		this.WBS = jQuery.extend(this.WBS, WBS);
	},
	...
	arrangeData: function(list, cond){
		...
		if(typeof cond === "undefined" || cond.reset) {
			...
			for (; idx < il; idx++) {
				if (L = list[idx]) {
					...
					if (L[keys.parent_key] == "0" || L[keys.parent_key] == 0 || L[keys.parent_key] == "") {
						L[keys.sub_list] = [];
						L.__subTreeLength = 0;
						L[keys.parent_hash] = "0";
						L[keys.child_hash] = "0_" + seq;
						L[keys.wbs_cd] = (seq + 1);
			...
			for (; idx < il; idx++) {
				if (L = list[idx]) {
					// rel_pointer에 저장할 데이터를 검증하고 추가합니다.
					if(L[keys.preceding_key]){
						this.rel_pointer[L[keys.child_key]] = L[keys.preceding_key];
					}
					if (typeof L[keys.parent_hash] == "undefined") {
						...
						if (pItem) {
							...
							L[keys.child_hash] = pHash + "_" + __subTreeLength;
							L[keys.wbs_cd] = pItem[keys.wbs_cd] + "." + (__subTreeLength + 1);
