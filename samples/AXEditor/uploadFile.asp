...
Upload.SetMaxSize (50*1024*1024), True
...
For Each File in Upload.Files
	...
	if LCase(FileExt) = ".jpg" or LCase(FileExt) = ".gif" or LCase(FileExt) = ".bmp" or LCase(FileExt) = ".png" then
		...
		call jpgResizeImage(strDir & TempFilename, 72, 72, 1.1, thb_strDir&"/"&thbName)
		thumb = thbfolderName&thbName
	end if

	response.write "{"
	response.write "ti:'"&enc(FileTitle)&"',"
	response.write "nm:'"&enc(TempFilename)&"',"
	response.write "ty:'"&enc(FileExt)&"',"
	response.write "sz:'"&File.size&"',"
	response.write "path:'"&enc(folderName)&"',"
	response.write "thumb:'"&enc(thumb)&"'"
	response.write "}"
