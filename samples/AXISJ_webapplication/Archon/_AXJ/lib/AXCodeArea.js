...
var AXCodeArea = Class.create(AXJ, {
	...
	bind: function(obj){
		...
		var textarea = $("#" + obj.id).find(".fieldsetBody");
	...
	appendAnchor: function(objID){
		var cfg = this.config;
		//trace("appendAnchor");
		var anchorNode = $("<div id=\"" + cfg.targetID + "_AX_"+objID+"\" class=\"" + cfg.anchorClassName + "\"></div>");
		var iobj = $("#" + objID).find(".fieldsetBody");
	...
	bindSourceCode: function(objID){
		var cfg = this.config;
		var textarea = $("#" + objID).find(".fieldsetBody");
		textarea.hide();
		$("#" + objID +"_AX_fieldsetBodyMirror").remove();
		
		var strs = textarea.val();
		strs = strs.replace(/\</g, "&lt;");
		strs = strs.replace(/\>/g, "&gt;");

		var mirror = $("<div id='"+objID +"_AX_fieldsetBodyMirror' class='fieldsetBodyMirror'><pre>"+strs+"</pre></div>");
		textarea.before(mirror);
		
		/*
		var textarea = $("#" + objID).find(".fieldsetBody").get(0);
		...
		lineNumber.push("</div>");
		$("#"+cfg.targetID + "_AX_"+objID).append(lineNumber.join(''));
