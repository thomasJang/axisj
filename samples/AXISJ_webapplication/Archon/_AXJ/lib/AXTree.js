...
var AXTree = Class.create(AXJ, {
	...
	initialize: function ($super) {
		...
		this.config.iconWidth = 25;
		this.config.indentWidth = 20;
		this.config.indentRatio = 0.7;
	...
	init: function () {
		...
		// 주요 타깃 설정
		this.gridBody = $("#" + cfg.targetID + "_AX_tree");
		this.scrollBody = $("#" + cfg.targetID + "_AX_treeScrollBody");
		this.colHead = $("#" + cfg.targetID + "_AX_treeColHead");
		this.body = $("#" + cfg.targetID + "_AX_treeBody");
		this.editor = $("#" + cfg.targetID + "_AX_treeEditor");
		...
		if (cfg.height != "auto") {
			...
			if (document.attachEvent) { //if IE (and Opera depending on user setting)
				//$M(cfg.targetID+"_AX_treeBody").detachEvent("on"+mousewheelevt, this.contentScrollScrollWheelBind);
				if ($M(cfg.targetID + "_AX_treeBody")) $M(cfg.targetID + "_AX_treeBody").attachEvent("on" + mousewheelevt, contentScrollScrollWheel);
			} else if (document.addEventListener) { //WC3 browsers
				///$M(cfg.targetID+"_AX_treeBody").removeEventListener(mousewheelevt, this.contentScrollScrollWheelBind, false);
				if ($M(cfg.targetID + "_AX_treeBody")) $M(cfg.targetID + "_AX_treeBody").addEventListener(mousewheelevt, contentScrollScrollWheel, false);
			}

			if (document.addEventListener) {
				//$M(cfg.targetID+"_AX_treeBody").removeEventListener("touchstart", this.contentScrollTouchstartBind, false);
				if ($M(cfg.targetID + "_AX_treeBody")) $M(cfg.targetID + "_AX_treeBody").addEventListener("touchstart", contentScrollTouchstart, false);
			}
		}
	...
	treeTargetSetSize: function (react) {
		...
		if (!react) $(window).bind("resize", this.contentScrollResize.bind(this));
	},
	resetHeight: function () {
		...
		try {
			...
			var scrollTrackYHeight = (cfg.xscroll) ? bodyHeight - $("#" + cfg.targetID + "_AX_scrollTrackXY").outerHeight() : bodyHeight;
			$("#" + cfg.targetID + "_AX_scrollTrackY").css({ height: scrollTrackYHeight });

			var scrollYHandleHeight = ((bodyHeight - $("#" + cfg.targetID + "_AX_scrollTrackXY").outerHeight()) * scrollTrackYHeight) / scrollHeight;
	...
	checkedColSeq: function (colSeq, checked) {
		...
		$("#" + cfg.targetID + "_AX_fixedColHead").find(".treeCheckBox_colHead_colSeq" + colSeq).each(function () {
			this.checked = checked;
		});
	...
	colHeadResizerMouseDown: function (event) {
		...
		$(document.body).bind("mousemove.AXTree", this.colHeadResizerMouseMoveBind);
		$(document.body).bind("mouseup.AXTree", this.colHeadResizerMouseUpBind);
		$(document.body).bind("mouseleave.AXTree", this.colHeadResizerMouseUpBind);
		$("iframe").bind("mouseover.AXTree", this.colHeadResizerMouseUpBind);
	...
	colHeadResizerEnd: function () {
		...
		$(document.body).unbind("mousemove.AXTree");
		$(document.body).unbind("mouseup.AXTree");
		$(document.body).unbind("mouseleave.AXTree");
		$("iframe").unbind("mouseover.AXTree");
	...
	colGroupListClick: function (event) {
		...
		var myTarget = this.getEventTarget({
			evt: eventTarget, evtIDs: eid,
			find: function (evt, evtIDs) { return ($(evt).hasClass("AXTreeColGroupListBoxItem") || $(evt).hasClass("colHeadTool")) ? true : false; }
		});
		// event target search ------------------------
	...
	setList: function (obj, positioning) {
		...
		$("#" + cfg.targetID + "_AX_scrollContent").css({ width: "auto" });
	...
	getFormatterValue: function (formatter, item, itemIndex, value, key, CH) {
		...
		if (formatter == "money") {
			...
			result = "<input type=\"checkbox\" name=\"" + CH.label + "\" class=\"treeCheckBox_body_colSeq" + CH.colSeq + "\" id=\"" + cfg.targetID + "_AX_checkboxItem_AX_" + CH.colSeq + "_AX_" + itemIndex + "\" value=\"" + value + "\" " + checked + disabled + " />";
		} else {
			var sendObj = {
				index: itemIndex,
				list: this.list,
	...
	getItem: function (itemIndex, item, isfix, hasTr) {
		...
		var trStyles = [];
		var isDisplay = item[cfg.reserveKeys.displayKey];
		if (!isDisplay) trStyles.push("display:none;");

		for (var r = 0; r < cfg.body.rows.length; r++) {
			var isLastTR = (cfg.body.rows.length - 1 == r);
			if (hasTrValue) tpo.push("<tr class=\"gridBodyTr gridBodyTr_" + itemIndex + " " + evenClassName + " " + hashClassName + "\" id=\"" + cfg.targetID + "_AX_tr_" + r + "_AX_" + (isfix || "n") + "_AX_" + itemIndex + "\" style=\"" + trStyles.join('') + "\">");
			var colCount = 0;
			$.each(cfg.body.rows[r], function (CHidx, CH) {
				if (CH.display && CH.colspan > 0) {
					...
					if (isfix == "n" || (isfix != undefined && colCount < (cfg.fixedColSeq + 1))) {
						...
						if (CH.indent) {
							if (item.__subTreeLength > 0)
								tpo.push("<a href=\"#AXexec\" class=\"bodyNodeIndent" + expandNodeClass + "\" id=\"" + cfg.targetID + "_AX_bodyNodeIndent_AX_" + r + "_AX_" + CHidx + "_AX_" + itemIndex + "\" style=\"" + CH.align + ":" + (indentWidth - 20) + "px;\"></a>");
							else
								tpo.push("<a href=\"#AXexec\" class=\"bodyNodeIndent" + expandNodeClass + "\" id=\"" + cfg.targetID + "_AX_bodyNodeIndent_AX_" + r + "_AX_" + CHidx + "_AX_" + itemIndex + "\" style=\"" + CH.align + ":" + (indentWidth - 20) + "px;display:none;\"></a>");
						}
						if (CH.getIconClass) {
	...
	printList: function () {
		...
		$.each(this.list, function (lidx, L) {
			if(L.__checked){
				...
				for (var l = hashs.length - 1; l > -1; l--) {
					...
					if(myTree) myTree.__checked = true;
				};
			}
		});
		...
		this.body.find(".gridBodyTr").bind("dblclick", this.gridBodyDBLClickBind);
		this.contentScrollResize();
	...
	updateList: function (itemIndex, item) {
		...
		if (this.hasFixed) {
			$("#" + cfg.targetID + "_AX_fixedTbody").find(".gridBodyTr_" + itemIndex).html(npo);
		}
		...
		this.body.find(".gridBodyTr").unbind("dblclick", this.gridBodyDBLClickBind);
	...
	expandToggleList: function (itemIndex, item, r, c) {
		...
		if (item[cfg.reserveKeys.openKey]) {
			...
			for (var r = 0; r < cfg.body.rows.length; r++) {
				$("#" + cfg.targetID + "_AX_tr_" + r + "_AX_n_AX_" + itemIndex).find(".bodyNodeIndent").removeClass("expand");
			}
			...
			for (var r = 0; r < cfg.body.rows.length; r++) {
				$("#" + cfg.targetID + "_AX_tr_" + r + "_AX_n_AX_" + itemIndex).find(".bodyNodeIndent").addClass("expand");
			}
			...
			var getAddHashs = function (_tree, pfHash) {
				...
				$.each(_tree, function (treeIndex, nTree) {
	...
	gridBodyDBLClick: function (event) {
		...
		var myTarget = this.getEventTarget({
			evt: eventTarget, evtIDs: eid,
			until: function (evt, evtIDs) { return ($(evt.parentNode).hasClass("gridBodyTr")) ? true : false; },
			find: function (evt, evtIDs) { return ($(evt).hasClass("bodyTd")) ? true : false; }
		});
		// event target search ------------------------
	...
	gridCheckClick: function (event) {
		...
		if (checked) { // 체크박스가 체크 된경우
			...
			var childIndex = [];
			$.each(this.list, function (lidx, list) {
				if (!this.isRoot) {
			...
			if (!parentIsRootTree) {
				...
				/*
				$.each(ptree[reserveKeys.subTree], function(){
			...
			if (checkboxRelationFixed) {
				...
				for (var l = hashs.length - 1; l > -1; l--) {
					...
					$.each(myTree[reserveKeys.subTree], function () {
	...
	contentScrollResize: function (resetLeft) {
		...
		if (cfg.width == "auto") scrollWidth = $("#" + cfg.targetID + "_AX_scrollContent").find("table.treeBodyTable").width().number();
		...
		if (bodyHeight < scrollHeight && cfg.height != "auto") {
			...
			if (cfg.width == "auto") {
				//trace(scrollWidth);
				//trace($("#" + cfg.targetID + "_AX_scrollContent").width());
				scrollWidth += 50;
				$("#" + cfg.targetID + "_AX_scrollContent").css({ width: scrollWidth });
				//trace($("#" + cfg.targetID + "_AX_scrollContent").width());
			}
			...
			$("#" + cfg.targetID + "_AX_scrollTrackXY").hide();
			$("#" + cfg.targetID + "_AX_scrollTrackY").hide();
	...
	contentScrollScrollSync: function (pos) {
		...
		if (pos.left != undefined) {
			...
			$("#" + cfg.targetID + "_AX_treeColHead").css({ left: -L });
	...
	contentScrollScrollReady: function (event) {
		...
		$(document.body).bind("mousemove.AXTree", this.contentScrollScrollMoveBind);
		$(document.body).bind("mouseup.AXTree", this.contentScrollScrollEndBind);
		$(document.body).bind("mouseleave.AXTree", this.contentScrollScrollEndBind);
		$("iframe").bind("mouseover.AXTree", this.contentScrollScrollEndBind);
	...
	setLoading: function (itemIndex, item) {
		var cfg = this.config;
		for (var r = 0; r < cfg.body.rows.length; r++) {
			$("#" + cfg.targetID + "_AX_tr_" + r + "_AX_n_AX_" + itemIndex).find(".bodyNodeIndent").addClass("loading");
		}
	},
	endLoading: function (itemIndex, item) {
		var cfg = this.config;
		for (var r = 0; r < cfg.body.rows.length; r++) {
			$("#" + cfg.targetID + "_AX_tr_" + r + "_AX_n_AX_" + itemIndex).find(".bodyNodeIndent").removeClass("loading");
		}
	},
	...
	removeTree: function (itemIndex, item) {
		...
		if (pItem) {
			...
			if (__subTreeLength == 0) {
				var lastR = cfg.body.rows.length - 1;
				$("#" + cfg.targetID + "_AX_tr_" + lastR + "_AX_n_AX_" + pItem.itemIndex).find(".bodyNodeIndent").hide();
				$("#" + cfg.targetID + "_AX_tr_" + lastR + "_AX_f_AX_" + pItem.itemIndex).find(".bodyNodeIndent").hide();
				$("#" + cfg.targetID + "_AX_tr_" + lastR + "_AX_n_AX_" + pItem.itemIndex).find(".bodyNodeIndent").removeClass("expand");
				$("#" + cfg.targetID + "_AX_tr_" + lastR + "_AX_f_AX_" + pItem.itemIndex).find(".bodyNodeIndent").removeClass("expand");
			}
		}
	...
	moveTree: function (Option) {
		...
		if (moveObj && targetObj) {
			...
			$(document.body).bind("keydown", this.eventForDocument);
	...
	moveTreeExec: function (moveIndex, targetIndex) {
		...
		$.each(move_Tree_parent_subTree, function (subTreeIndex, ST) {
	...
	cancelMove: function () {
		...
		$(document.body).unbind("keydown", this.eventForDocument);
	},
	addClassItem: function (Option) {
		var body = this.body;

		$.each(this.list, function (listIndex, item) {
			if (Option.addClass) {
	...
	convertTreeToHashList: function (List) {
		...
		for (var idx = 0; idx < List.length; idx++) {
			...
			$.each(item, function (k, v) {
				var addOk = true;
				$.each(reserveKeys, function (kk, vv) {
					if (k == vv) {
						addOk = false;
						return false;
					}
        ...
        this.hashList = [];
        var pushItem = {isRoot:true};
        pushItem[reserveKeys.hashKey] = "0";
        pushItem[reserveKeys.parentHashKey] = "";
        pushItem[reserveKeys.openKey] = true;
        pushItem._subTreeLength = Tree.length;
        this.hashList.push(pushItem);
        this.getSubTreeList(pushItem[reserveKeys.hashKey], pushItem[reserveKeys.openKey], Tree);
        return this.hashList;
        */
	},
	...
	getSelectedListParent: function () {
		...
		if (this.selectedRow != undefined && this.selectedRow != null && this.selectedRow.length > 0) {
			...
			if (pno != "0") {
				$.each(this.list, function (findIndex, findItem) {
