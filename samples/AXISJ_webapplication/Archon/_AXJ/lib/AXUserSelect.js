...
var AXUserSelect = Class.create(AXJ, {
    ...
    init: function() {
        ...
        if (!config.containerID || !$("#" + config.containerID).get(0)) {
    ...
    push: function(ds) {
        ...
        $("#" + config.containerID + "_UserSelectBox").append(po.join(''));
    ...
    onDrop: function(res) {
        ...
        var dragCollect = this.dragCollect.bind(this);
        $(res.dragItem).fadeOut("fast", function() {
            $("#" + config.containerID + "_UserSelectBox").append(this); //예외 경우
    ...
    empty: function() {
        var config = this.config;
        $("#" + config.containerID + "_UserSelectBox").empty();
        this.ds = [];
        this.dragCollect();
    },
    getDS: function() {
        var config = this.config;
        var myDS = [];
        $("#" + config.containerID + "_UserSelectBox").find(".readyDrag").each(function() {
            var id = this.id.split(/\_/g).last();
            var nm = $("#" + config.containerID + "userSelectItemNM_" + id).val();
            var desc = $("#" + config.containerID + "userSelectItemDESC_" + id).val();
