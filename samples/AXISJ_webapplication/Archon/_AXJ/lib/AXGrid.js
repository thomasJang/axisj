...
var AXGrid = Class.create(AXJ, {
	...
	defineConfig: function (rewrite) {
		...
		$.each(cfg.colGroup, function (cidx, CG) {
		...
		if (cfg.colHead.rows) {
			...
			for (var r = 0; r < cfg.colHead.rows.length; r++) {
				var colLen = 0;
				$.each(cfg.colHead.rows[r], function (CHidx, CH) {
			...
			var appendPosToColHeadMap = function (r, c, posR, position) { //
				...
				for (var rr = posR; rr < (posR + r) ; rr++) {
					if(r > 1){
						if(rr > 0 && startPosition != null) nC = startPosition;
					}
					var tC = c; //컬럼 루프횟수
			...
			$.each(cfg.colHead._maps.last(), function (midx, m) {
		...
		if (cfg.body.rows) {
			...
			for (var r = 0; r < cfg.body.rows.length; r++) {
				var colLen = 0;
				$.each(cfg.body.rows[r], function (CHidx, CH) {
					if (CH.colspan == undefined || CH.colspan == null) {
						CH.colspan = 1;
						CH._colspan = 1;
					} else {
			...
			$.each(cfg.body._maps.last(), function (midx, m) {
			...
			$.each(cfg.colGroup, function (cidx, CG) {
				var adder = {
					...
					display: CG.display, checked: CG.checked, formatter: CG.formatter,
					tooltip: CG.tooltip
				};
		...
		if (cfg.body.marker) {
			if (cfg.body.marker.rows) {
				...
				for (var r = 0; r < cfg.body.marker.rows.length; r++) {
					var colLen = 0;
					$.each(cfg.body.marker.rows[r], function (CHidx, CH) {
						if (CH.rowspan == undefined || CH.rowspan == null) CH.rowspan = 1;
						if (CH.colspan == undefined || CH.colspan == null) {
							CH.colspan = 1;
							CH._colspan = 1;
				...
				//colHead._maps 마지막 줄에 해당하는 cfg.colHead.rows 에 속성부여
				$.each(cfg.body.marker._maps.last(), function (midx, m) {
		...
		if (cfg.head) {
			...
			for (var r = 0; r < cfg.head.rows.length; r++) {
				var colLen = 0;
				$.each(cfg.head.rows[r], function (CHidx, CH) {
					if (CH.rowspan == undefined || CH.rowspan == null) CH.rowspan = 1;
					if (CH.colspan == undefined || CH.colspan == null) {
						CH.colspan = 1;
						CH._colspan = 1;
			...
			//colHead._maps 마지막 줄에 해당하는 cfg.colHead.rows 에 속성부여
			$.each(cfg.head._maps.last(), function (midx, m) {
		...
		if (cfg.foot) {
			...
			for (var r = 0; r < cfg.foot.rows.length; r++) {
				var colLen = 0;
				$.each(cfg.foot.rows[r], function (CHidx, CH) {
					if (CH.rowspan == undefined || CH.rowspan == null) CH.rowspan = 1;
					if (CH.colspan == undefined || CH.colspan == null) {
						CH.colspan = 1;
						CH._colspan = 1;
			...
			//colHead._maps 마지막 줄에 해당하는 cfg.colHead.rows 에 속성부여
			$.each(cfg.foot._maps.last(), function (midx, m) {
		...
		if (cfg.editor) {
			if (cfg.editor.rows) {
				...
				for (var r = 0; r < cfg.editor.rows.length; r++) {
					var colLen = 0;
					$.each(cfg.editor.rows[r], function (CHidx, CH) {
						if (CH.rowspan == undefined || CH.rowspan == null) CH.rowspan = 1;
						if (CH.colspan == undefined || CH.colspan == null) {
							CH.colspan = 1;
							CH._colspan = 1;
				...
				//colHead._maps 마지막 줄에 해당하는 cfg.colHead.rows 에 속성부여
				$.each(cfg.editor._maps.last(), function (midx, m) {
		...
		if (cfg.fixedColSeq != undefined && cfg.fixedColSeq != null) {
			...
			$.each(cfg.colHead._maps, function (midx, m) {
				$.each(m, function (cidx, c) {
					if (c) {
						if ((fixedColSeq + 1) > cidx) cfg.colHead.rows[c.r][c.c].isFixedCell = true;
					}
				});
			});
			$.each(cfg.body._maps, function (midx, m) {
				$.each(m, function (cidx, c) {
					if (c) {
			...
			if (cfg.head) {
				$.each(cfg.head._maps, function (midx, m) {
					$.each(m, function (cidx, c) {
						if (c) {
			...
			if (cfg.foot) {
				$.each(cfg.foot._maps, function (midx, m) {
					$.each(m, function (cidx, c) {
						if (c) {
			...
			if (cfg.body.marker) {
				if (cfg.body.marker.rows) {
					$.each(cfg.body.marker._maps, function (midx, m) {
						$.each(m, function (cidx, c) {
							if (c) {
			...
			if (cfg.editor) {
				if (cfg.editor.rows) {
					$.each(cfg.editor._maps, function (midx, m) {
						$.each(m, function (cidx, c) {
							if (c) {
	...
	init: function () {
		...
		var targetInnerHeight = $("#" + cfg.targetID).innerHeight();
		...
		// 주요 타깃 설정
		this.gridBody = $("#" + cfg.targetID + "_AX_grid");
		this.scrollBody = $("#" + cfg.targetID + "_AX_gridScrollBody");
		this.colHead = $("#" + cfg.targetID + "_AX_gridColHead");
		this.body = $("#" + cfg.targetID + "_AX_gridBody");
		this.editor = $("#" + cfg.targetID + "_AX_gridEditor");

		this.pageBody = $("#" + cfg.targetID + "_AX_gridPageBody");
		this.pagingUnit = $("#" + cfg.targetID + "_AX_gridPagingUnit");
		this.status = $("#" + cfg.targetID + "_AX_gridStatus");
		this.scroller = $("#" + cfg.targetID + "_AX_gridScroller");
		...
		if (document.attachEvent) { //if IE (and Opera depending on user setting)
			//$M(cfg.targetID+"_AX_gridBody").detachEvent("on"+mousewheelevt, this.contentScrollScrollWheelBind);
			if ($M(cfg.targetID + "_AX_gridBody")) $M(cfg.targetID + "_AX_gridBody").attachEvent("on" + mousewheelevt, contentScrollScrollWheel);
		} else if (document.addEventListener) { //WC3 browsers
			///$M(cfg.targetID+"_AX_gridBody").removeEventListener(mousewheelevt, this.contentScrollScrollWheelBind, false);
			if ($M(cfg.targetID + "_AX_gridBody")) $M(cfg.targetID + "_AX_gridBody").addEventListener(mousewheelevt, contentScrollScrollWheel, false);
		}

		if (document.addEventListener) {
			//$M(cfg.targetID+"_AX_gridBody").removeEventListener("touchstart", this.contentScrollTouchstartBind, false);
			if ($M(cfg.targetID + "_AX_gridBody")) $M(cfg.targetID + "_AX_gridBody").addEventListener("touchstart", contentScrollTouchstart, false);
		}

		$("#" + cfg.targetID).bind("keydown", this.onKeydown.bind(this));
		...
		if (cfg.contextMenu) {
			...
			$("#" + cfg.targetID).bind("contextmenu", this.onContextmenu.bind(this));
		}
	...
	gridTargetSetSize: function (react) { // AXgridScrollBody 안쪽의 높이와 관련된 요소 설정
		...
		if (cfg.height == "auto") {
			...
			var scrollBodyHeight = $("#" + cfg.targetID + "_AX_scrollContent").height();
	...
	checkedColSeq: function (colSeq, checked) {
		...
		$("#" + cfg.targetID + "_AX_fixedColHead").find(".gridCheckBox_colHead_colSeq" + colSeq).each(function () {
			this.checked = checked;
	...
	onContextmenu: function (event) {
		...
		var myTarget = this.getEventTarget({
			...
			until: function (evt, evtIDs) { return ($(evt.parentNode).hasClass("gridBodyTr")) ? true : false; },
			find: function (evt, evtIDs) { return ($(evt).hasClass("bodyTd") || $(evt).hasClass("bodyNodeIndent")) ? true : false; }
		});
		// event target search ------------------------
		...
		if (myTarget) {
			...
			if (this.selectedRow.length > 0) {
				var body = this.body;
				$.each(this.selectedRow, function () {
	...
	copyData: function () {
		var cfg = this.config;
		var copyOut = [];
		$.each(this.selectedCells, function (index, n) {
			var html = $("#" + n).find(".bodyNode").html();
			copyOut.push(html + '\t');
		});
	...
	setColHead: function () {
		...
		$("#" + cfg.targetID + "_AX_fixedColHead").remove();
		...
		this.colHead.find(".colHeadResizer").each(function () {
			...
			var rowspan = $("#" + tdID).attr("rowspan");
			var valign = $("#" + tdID).attr("valign");
			if (!rowspan) rowspan = 1;
			var tdHeight = $("#" + tdID).height();
			$(this).css({ height: tdHeight });
			$(this).parent().css({ height: tdHeight });
			if (rowspan > 1) {
				var cellMarginTop = 0;
				if (valign == "bottom") cellMarginTop = (tdHeight - $("#" + txtID).outerHeight()) + 5;
				if (valign == "middle") cellMarginTop = (tdHeight - $("#" + txtID).outerHeight()) / 2 + 5;
				$("#" + txtID).css({ "padding-top": cellMarginTop + "px" });
				$("#" + toolID).css({ "top": (cellMarginTop - 5) + "px" });
			}
		});
		...
		if (this.hasFixed) { //fixedColHead에 대한 바인딩 및 처리
			this.fixedColHead = $("#" + cfg.targetID + "_AX_fixedColHead");
	...
	colHeadMouseOver: function (event) {
		...
		var myTarget = this.getEventTarget({
			evt: eventTarget, evtIDs: eid,
			until: function (evt, evtIDs) { return ($(evt.parentNode).hasClass("AXGridColHead")) ? true : false; },
			find: function (evt, evtIDs) { return ($(evt).hasClass("colHeadTd")) ? true : false; }
		});
		// event target search ------------------------
	...
	colHeadResizerMouseDown: function (event) {
		...
		var offset = $("#" + cfg.targetID + "_AX_colHead_AX_" + colHeadR + "_AX_" + colHeadC).find(".tdRelBlock").position();
		var relBlockWidth = $("#" + cfg.targetID + "_AX_colHead_AX_" + colHeadR + "_AX_" + colHeadC).find(".tdRelBlock").width();
		var rightPosition = offset.left.number() + relBlockWidth.number();
		var blockWidth = $("#" + cfg.targetID + "_AX_col_AX_" + colSeq + "_AX_CH").attr("width");
		...
		$(document.body).bind("mousemove.AXGrid", this.colHeadResizerMouseMoveBind);
		$(document.body).bind("mouseup.AXGrid", this.colHeadResizerMouseUpBind);
		$(document.body).bind("mouseleave.AXGrid", this.colHeadResizerMouseUpBind);
	...
	colHeadResizerMove: function (event) {
		...
		$("#" + cfg.targetID + "_AX_col_AX_" + this.colResizeTarget.colSeq + "_AX_CH").attr("width", newWidth);
		$("#" + cfg.targetID + "_AX_col_AX_" + this.colResizeTarget.colSeq + "_AX_CB").attr("width", newWidth);
		$("#" + cfg.targetID + "_AX_col_AX_" + this.colResizeTarget.colSeq + "_AX_EB").attr("width", newWidth);
		...
		if (this.hasFixed) {
			...
			$("#" + cfg.targetID + "_AX_col_AX_" + this.colResizeTarget.colSeq + "_AX_FC").attr("width", newWidth);
			$("#" + cfg.targetID + "_AX_col_AX_" + this.colResizeTarget.colSeq + "_AX_FB").attr("width", newWidth);
			$("#" + cfg.targetID + "_AX_col_AX_" + this.colResizeTarget.colSeq + "_AX_FE").attr("width", newWidth);
			...
			$("#" + cfg.targetID + "_AX_fixedColHead").css({ width: fixedColWidth });
			$("#" + cfg.targetID + "_AX_fixedColHead").find(".colHeadTable").css({ width: fixedColWidth });
			$("#" + cfg.targetID + "_AX_fixedScrollContent").css({ width: fixedColWidth });
			$("#" + cfg.targetID + "_AX_fixedScrollContent").find(".gridFixedBodyTable").css({ width: fixedColWidth });
			$("#" + cfg.targetID + "_AX_fixedEditorContent").css({ width: fixedColWidth });
			$("#" + cfg.targetID + "_AX_fixedEditorContent").find(".gridFixedBodyTable").css({ width: fixedColWidth });
			//}
		...
		if (this.editorOpend) {
			...
			for (var _m = 0; _m < cfg.editor._maps.length; _m++) {
				...
				//trace(formID);
				if ($M(formID)) if ($M(formID).tagName == "INPUT") AXInput.alignAnchor(formID);
			}
		}
	...
	colHeadResizerEnd: function () {
		...
		$(document.body).unbind("mousemove.AXGrid");
		$(document.body).unbind("mouseup.AXGrid");
		$(document.body).unbind("mouseleave.AXGrid");
	...
	colHeadNodeClick: function (event) {
		...
		if ($(eventTarget).hasClass("colHeadTdCheck")) {
			this.colHeadCheckBoxClick(event);
			return; // checkbox block click
		}
		if ($(eventTarget).hasClass("gridCheckBox")) return; // checkbox click
		...
		if (myColHead.colSeq == undefined || myColHead.colSeq == null) {
			...
			if (this.nowSortHeadID) {
				if (this.nowSortHeadID != tdID) {
					$("#" + this.nowSortHeadID).removeClass("sortDesc");
					$("#" + this.nowSortHeadID).removeClass("sortAsc");
					this.nowSortHeadObj.sort = undefined;
				}
			}
			//trace(myColHead);
			if (cfg.colHead.rows[colHeadR][colHeadC].sort == "desc") $("#" + tdID).removeClass("sortDesc");
			else $("#" + tdID).removeClass("sortAsc");
			...
			//sort 처리하기
			if (nsort == "desc") {
				$("#" + tdID).addClass("sortDesc");
			} else {
				$("#" + tdID).addClass("sortAsc");
			}
	...
	colHeadToolClick: function (event) {
		...
		$("#" + cfg.targetID + "_AX_colHeadMenu").remove();
		...
		$("#" + cfg.targetID + "_AX_colHeadMenu").css(css);
		...
		$(document).bind("click", this.colGroupListClickBind);
		$(document).bind("keydown", this.colGroupListClickBind);
		/* colGroup click bind ~~~~~~~~~~~~~~~~~~~ */
	},
	colGroupListClick: function (event) {
		...
		if (event.keyCode == AXUtil.Event.KEY_ESC) {
			$("#" + cfg.targetID + "_AX_colHeadMenu").remove();
			$(document).unbind("keydown", this.colGroupListClickBind);
			$(document).unbind("click", this.colGroupListClickBind);
			return;
		}
		...
		var myTarget = this.getEventTarget({
			evt: eventTarget, evtIDs: eid,
			find: function (evt, evtIDs) { return ($(evt).hasClass("AXGridColGroupListBoxItem") || $(evt).hasClass("colHeadTool")) ? true : false; }
		});
		// event target search ------------------------

		if (myTarget) {
			if ($(myTarget).hasClass("colHeadTool")) return;
			//colHeadTool ready
			var targetID = myTarget.id;
	...
	colHeadCheckBoxClick: function (event) {
		...
		if ($(eventTarget).hasClass("colHeadTdCheck")) {
			eventTarget = $(eventTarget).find("input").get(0);
		...
		var getValueForSort = function (item, itemIndex) {
			if (myColHead.formatter) {
				...
				if (myColHead.formatter == "money") {
					...
					if ($.isNumeric(moneyCheck)) result = result.number();
				}
				return result;
			} else {
	...
	setBody: function (list, rewrite) {
		...
		//if(!cfg.xscroll) $("#"+cfg.targetID+"_AX_scrollTrackX").hide();

		// scroll event bind
		$("#" + cfg.targetID + "_AX_scrollYHandle").unbind();
		$("#" + cfg.targetID + "_AX_scrollXHandle").unbind();

		$("#" + cfg.targetID + "_AX_scrollYHandle").bind("mousedown", this.contentScrollScrollReady.bind(this));
		$("#" + cfg.targetID + "_AX_scrollXHandle").bind("mousedown", this.contentScrollScrollReady.bind(this));
		// scroll event bind ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	},
	listLoadingDisplay: function () {
		...
		po.push("<td class=\"bodyNullTd\"><div class=\"tdRelBlock\">&nbsp;</div></td>");
		po.push("</tr>");
		$("#" + cfg.targetID + "_AX_tbody").html(po.join(''));
		po = [];
		po.push("<tr class=\"noListTr\">");
		po.push("<td colspan=\"" + (this.showColLen) + "\">");
		po.push("</td>");
		po.push("</tr>");
		$("#" + cfg.targetID + "_AX_fixedTbody").html(po.join(''));
	},
	...
	reloadList: function () {
		...
		if (this.ajaxInfo) {
			...
			var scrollTop = function () {
				//스크롤 위치 변경
				var scrollTop = 0;
				$("#" + cfg.targetID + "_AX_scrollContent").css({ top: scrollTop });
	...
	getFormatterValue: function (formatter, item, itemIndex, value, key, CH) {
		...
		if (formatter == "money") {
			...
			result = "<input type=\"checkbox\" name=\"" + CH.label + "\" class=\"gridCheckBox_body_colSeq" + CH.colSeq + "\" id=\"" + cfg.targetID + "_AX_checkboxItem_AX_" + CH.colSeq + "_AX_" + itemIndex + "\" value=\"" + value + "\" " + checked + " />";
		} else {
			var sendObj = {
				index: itemIndex,
				list: this.list,
	...
	getIconItem: function (itemIndex, item, viewIconObj, cssObj) {
		...
		var format = viewIconObj.format.call({ index: itemIndex, item: item });
		...
		if (viewIconObj.buttons) {
			if (viewIconObj.buttons.items) {
				...
				$.each(viewIconObj.buttons.items, function (bidx, B) {
	...
	getMarkerDisplay: function (itemIndex, item) {
		...
		var markerDisplay = cfg.body.marker.display.call(sendObj, itemIndex, item);
	...
	printList: function () {
		...
		if (cfg.viewMode == "grid") {
			...
			$("#" + cfg.targetID + "_AX_gridBodyTable").show();
			$("#" + cfg.targetID + "_AX_gridBodyDiv").hide();
			...
			this.body.find(".gridBodyTr").bind("dblclick", this.gridBodyDBLClick.bind(this));

			//if (po.length == 0) this.redrawGrid();

		} else {

			$("#" + cfg.targetID + "_AX_gridBodyTable").hide();
			$("#" + cfg.targetID + "_AX_gridBodyDiv").show();
			...
			$("#" + cfg.targetID + "_AX_gridBodyDiv").html(po.join(''));

			this.body.find(".bodyViewIcon").bind("click", this.gridBodyClick.bind(this));
			this.body.find(".bodyViewIcon").bind("dblclick", this.gridBodyDBLClick.bind(this));
	...
	updateList: function (itemIndex, item) {
		...
		///
		$("#" + cfg.targetID + "_AX_tbody").find(".gridBodyTr_" + itemIndex).html(npo);
		if (this.hasFixed) {
			$("#" + cfg.targetID + "_AX_fixedTbody").find(".gridBodyTr_" + itemIndex).html(fpo);
		}
		////
		...
		$("#" + cfg.targetID + "_AX_tbody").find(".gridBodyTr_" + itemIndex).addClass(trAddClass);
		if (this.hasFixed) {
			$("#" + cfg.targetID + "_AX_fixedTbody").find(".gridBodyTr_" + itemIndex).addClass(trAddClass);;
		}
	...
	pushList: function (pushItem, insertIndex) {
		...
		if (insertIndex != null && insertIndex != undefined) {
			...
			var newList = [];
			$.each(this.list, function (listIndex, L) {
			...
			if (itemIndex == 0) {
				...
				var trlen = $("#" + cfg.targetID + "_AX_tbody").find(".gridBodyTr_" + (itemIndex.number() - 1)).length - 1;
				$("#" + cfg.targetID + "_AX_tbody").find(".gridBodyTr_" + (itemIndex.number() - 1)).each(function (idx, O) {
					if (idx == trlen) $(this).after(npo);
				});
				if (this.hasFixed) {
					$("#" + cfg.targetID + "_AX_fixedTbody").find(".gridBodyTr_" + (itemIndex - 1)).each(function (idx, O) {
						if (idx == trlen) $(this).after(fpo);
					});
				}

				$("#" + cfg.targetID + "_AX_tbody").find(".gridBodyTr_" + itemIndex).bind("mouseover", this.gridBodyOver.bind(this));
				$("#" + cfg.targetID + "_AX_tbody").find(".gridBodyTr_" + itemIndex).bind("mouseout", this.gridBodyOut.bind(this));
				$("#" + cfg.targetID + "_AX_tbody").find(".gridBodyTr_" + itemIndex).bind("click", this.gridBodyClick.bind(this));
				$("#" + cfg.targetID + "_AX_tbody").find(".gridBodyTr_" + itemIndex).bind("dblclick", this.gridBodyDBLClick.bind(this));
				if (this.hasFixed) {
					$("#" + cfg.targetID + "_AX_fixedTbody").find(".gridBodyTr_" + itemIndex).bind("mouseover", this.gridBodyOver.bind(this));
					$("#" + cfg.targetID + "_AX_fixedTbody").find(".gridBodyTr_" + itemIndex).bind("mouseout", this.gridBodyOut.bind(this));
					$("#" + cfg.targetID + "_AX_fixedTbody").find(".gridBodyTr_" + itemIndex).bind("click", this.gridBodyClick.bind(this));
					$("#" + cfg.targetID + "_AX_fixedTbody").find(".gridBodyTr_" + itemIndex).bind("dblclick", this.gridBodyDBLClick.bind(this));
				}
	...
	removeList: function (removeList) {
		...
		if (cfg.passiveMode) {
			...
			var _list = this.list;
			var collect = [];
			$.each(removeList, function (ridx, r) {
				$.each(_list, function (lidx, l) {
					var isDel = false;
					$.each(r, function (k, v) {
						if (l[k] == v) {
							isDel = true;
	...
	restoreList: function (restoreList) {
		...
		var _list = this.list;
		var collect = [];
		$.each(restoreList, function (ridx, r) {
			$.each(_list, function (lidx, l) {
				var isDel = false;
				$.each(r, function (k, v) {
					if (l[k] == v) {
	...
	gridBodyClick: function (event) {
		...
		var myTarget = this.getEventTarget({
			evt: eventTarget, evtIDs: eid,
			until: function (evt, evtIDs) { return ($(evt.parentNode).hasClass("gridBodyTr")) ? true : false; },
			find: function (evt, evtIDs) { return ($(evt).hasClass("bodyTd") || $(evt).hasClass("bodyViewIcon")) ? true : false; }
		});
		// event target search ------------------------
	...
	contentScrollResize: function (resetLeft) {
		...
		var bodyHeight = this.body.height() - $("#" + cfg.targetID + "_AX_scrollTrackXY").outerHeight();
		var scrollHeight = $("#" + cfg.targetID + "_AX_scrollContent").height();

		var bodyWidth = this.body.width();		
		var scrollWidth = (this.colWidth > bodyWidth) ? this.colWidth : bodyWidth;

		$("#" + cfg.targetID + "_AX_scrollContent").css({ width: scrollWidth });
		this.colHead.css({ width: scrollWidth + $("#" + cfg.targetID + "_AX_scrollTrackXY").outerWidth() });  // colHead width 재정의

		if (this.hasEditor) this.editor.css({ width: scrollWidth });

		if (resetLeft != false) {
			$("#" + cfg.targetID + "_AX_scrollContent").css({ left: 0 });
			$("#" + cfg.targetID + "_AX_gridColHead").css({ left: 0 });
			$("#" + cfg.targetID + "_AX_scrollXHandle").css({ left: 0 });
			if (this.hasEditor) $("#" + cfg.targetID + "_AX_editorContent").css({ left: 0 });
		}

		if (bodyHeight < scrollHeight && cfg.height != "auto") {
			$("#" + cfg.targetID + "_AX_scrollTrackXY").show();
			$("#" + cfg.targetID + "_AX_scrollTrackY").show();

			var scrollTrackYHeight = bodyHeight;
			$("#" + cfg.targetID + "_AX_scrollTrackY").css({ height: scrollTrackYHeight });

			var scrollYHandleHeight = ((bodyHeight) * scrollTrackYHeight) / scrollHeight;
			$("#" + cfg.targetID + "_AX_scrollYHandle").css({ height: scrollYHandleHeight - 2 });
		}
		...
		if (scrollWidth > bodyWidth && cfg.xscroll) {
			...
			$("#" + cfg.targetID + "_AX_scrollTrackXY").show();
			$("#" + cfg.targetID + "_AX_scrollTrackX").show();

			var scrollTrackXWidth = bodyWidth - $("#" + cfg.targetID + "_AX_scrollTrackXY").outerWidth();
			$("#" + cfg.targetID + "_AX_scrollTrackX").css({ width: scrollTrackXWidth });
			var scrollXHandleWidth = ((bodyWidth - $("#" + cfg.targetID + "_AX_scrollTrackXY").outerWidth()) * scrollTrackXWidth) / scrollWidth;

			$("#" + cfg.targetID + "_AX_scrollXHandle").css({ width: scrollXHandleWidth - 2 });

			// cfg.height == "auto" 길이 늘이기
			if (cfg.height == "auto") {
				var colHeadHeight = this.colHead.outerHeight();
				var scrollBodyHeight = $("#" + cfg.targetID + "_AX_scrollContent").height();
				var scrollTrackXYHeight = $("#" + cfg.targetID + "_AX_scrollTrackXY").outerHeight()
			...
			$("#" + cfg.targetID + "_AX_scrollTrackX").hide();
			if (cfg.height == "auto") $("#" + cfg.targetID + "_AX_scrollTrackXY").hide();
		...
		if (pos.left != undefined) {
			if (!this.contentScrollXAttr) {
				...
				this.contentScrollXAttr = {
					...
					scrollTrackXWidth: $("#" + cfg.targetID + "_AX_scrollTrackX").width(),
					scrollXHandleWidth: $("#" + cfg.targetID + "_AX_scrollXHandle").outerHeight()
				};
			}

			//x 변경
			var L = (this.contentScrollXAttr.scrollWidth * (pos.left) / this.contentScrollXAttr.scrollTrackXWidth).round(0);
			$("#" + cfg.targetID + "_AX_scrollContent").css({ left: -L });
			$("#" + cfg.targetID + "_AX_gridColHead").css({ left: -L });
			if (this.hasEditor) $("#" + cfg.targetID + "_AX_editorContent").css({ left: -L });
			//trace({top:-L});
		} else {
			...
			if (!this.contentScrollYAttr) {
				this.contentScrollYAttr = {
					...
					scrollHeight: $("#" + cfg.targetID + "_AX_scrollContent").height(),
					scrollTrackYHeight: $("#" + cfg.targetID + "_AX_scrollTrackY").height(),
					scrollYHandleHeight: $("#" + cfg.targetID + "_AX_scrollYHandle").outerHeight()
				};
			}

			//y 변경
			var T = (this.contentScrollYAttr.scrollHeight * (pos.top) / this.contentScrollYAttr.scrollTrackYHeight).round(0);
			$("#" + cfg.targetID + "_AX_scrollContent").css({ top: -T });
			if ($M(cfg.targetID + "_AX_fixedScrollContent")) $("#" + cfg.targetID + "_AX_fixedScrollContent").css({ top: -T });
			if (this.editorOpend) {
	...
	contentScrollContentSync: function (pos) {
		...
		if (pos.left != undefined) {
			...
			if (!this.contentScrollYAttr) {
				this.contentScrollYAttr = {
					bodyHeight: this.body.height() - $("#" + cfg.targetID + "_AX_scrollTrackXY").outerHeight(),
					scrollHeight: $("#" + cfg.targetID + "_AX_scrollContent").height(),
					scrollTrackYHeight: $("#" + cfg.targetID + "_AX_scrollTrackY").height(),
					scrollYHandleHeight: $("#" + cfg.targetID + "_AX_scrollYHandle").outerHeight()
				};
			...
			if ($M(cfg.targetID + "_AX_fixedScrollContent")) $("#" + cfg.targetID + "_AX_fixedScrollContent").css({ top: pos.top });
			if (this.editorOpend) {
	...
	contentScrollScrollReady: function (event) {
		...
		this.contentScrollAttrs.x = $(event.target).position().left - pos.x;
		this.contentScrollAttrs.y = $(event.target).position().top - pos.y;
		this.contentScrollAttrs.handleWidth = $(event.target).outerWidth();
		this.contentScrollAttrs.handleHeight = $(event.target).outerHeight();
		this.contentScrollAttrs.trackWidth = $("#" + this.contentScrollAttrs.scrollTrack).width();
		this.contentScrollAttrs.trackHeight = $("#" + this.contentScrollAttrs.scrollTrack).height();
		...
		$(document.body).bind("mousemove.AXGrid", this.contentScrollScrollMoveBind);
		$(document.body).bind("mouseup.AXGrid", this.contentScrollScrollEndBind);
		$(document.body).bind("mouseleave.AXGrid", this.contentScrollScrollEndBind);
	...
	contentScrollScrollMove: function (event) {
		...
		if (handleName == "scrollYHandle") {
			...
			$("#" + cfg.targetID + "_AX_" + handleName).css({ top: handleTop });
			...
			$("#" + cfg.targetID + "_AX_" + handleName).css({ left: handleLeft });
	...
	contentScrollScrollEnd: function (event) {
		...
		$(document.body).removeAttr("onselectstart");
		$(document.body).removeClass("AXUserSelectNone");
		$("#" + cfg.targetID + "_AX_" + this.contentScrollAttrs.handleName).removeClass("hover");
	},
	contentScrollScrollWheel: function (e) {
		...
		var bodyHeight = this.body.height() - $("#" + cfg.targetID + "_AX_scrollTrackXY").outerHeight();
		var scrollTop = $("#" + cfg.targetID + "_AX_scrollContent").position().top;
		var scrollHeight = $("#" + cfg.targetID + "_AX_scrollContent").height();

		//var handleTop = $("#"+cfg.targetID+"_AX_scrollYHandle").position().top; i want this value
		var handleHeight = $("#" + cfg.targetID + "_AX_scrollYHandle").outerHeight();
		var trackHeight = $("#" + cfg.targetID + "_AX_scrollTrackY").height();
	...
	contentScrollTouchstart: function (e) {
		...
		this.contentScrollIDOffset = $("#" + cfg.targetID + "_AX_gridBody").offset();
		this.contentScrollXAttr = null;
		this.contentScrollYAttr = null;

		var pos = this.getTouchPositionToContentScroll(event);

		var YhandleTop = $("#" + cfg.targetID + "_AX_scrollYHandle").position().top;
		var YhandleHeight = $("#" + cfg.targetID + "_AX_scrollYHandle").outerHeight();
		var YtrackHeight = $("#" + cfg.targetID + "_AX_scrollTrackY").height();

		$("#" + cfg.targetID + "_AX_scrollYHandle").addClass("hover");

		var XhandleTop = $("#" + cfg.targetID + "_AX_scrollXHandle").position().left;
		var XhandleHeight = $("#" + cfg.targetID + "_AX_scrollXHandle").outerWidth();
		var XtrackHeight = $("#" + cfg.targetID + "_AX_scrollTrackX").width();

		$("#" + cfg.targetID + "_AX_scrollXHandle").addClass("hover");
	...
	contentScrollTouchMove: function (e) {
		...
		if (this.contentScrollTouchMoved) {
			...
			var htop = $("#" + cfg.targetID + "_AX_scrollYHandle").position().top;
			...
			if (this.show_scrollTrackX) {
				var hleft = $("#" + cfg.targetID + "_AX_scrollXHandle").position().left;
				...
				if ((hleft - handleLeft).abs() > 2) {
					$("#" + cfg.targetID + "_AX_scrollXHandle").css({ left: handleLeft });
					this.contentScrollScrollSync({ left: handleLeft });
				}
			}
	...
	contentScrollTouchEnd: function (e) {
		...
		if (this.contentScrollTouchMoved) {
			...
			$("#" + cfg.targetID + "_AX_scrollXHandle").removeClass("hover");
			$("#" + cfg.targetID + "_AX_scrollYHandle").removeClass("hover");
	...
	printHead: function () {
		...
		$("#" + cfg.targetID + "_AX_thead").html(po.join(''));
		if (this.hasFixed) {
			po = [];
			po.push(getDataSet(this.dataSet, "fix"));
			$("#" + cfg.targetID + "_AX_fixedThead").html(po.join(''));
		}
	},
	printFoot: function () {
		var cfg = this.config;
		...
		var po = [];
		po.push(getDataSet(this.dataSet));
		$("#" + cfg.targetID + "_AX_tfoot").html(po.join(''));
		if (this.hasFixed) {
			po = [];
			po.push(getDataSet(this.dataSet, "fix"));
			$("#" + cfg.targetID + "_AX_fixedTfoot").html(po.join(''));
		}
	},
	/* head & foot 영역 ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/

	/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ editor 영역  */
	getEditorFormatterValue: function (formatter, dataSet, value, key, CH) {
		var cfg = this.config;
		var result;
		if (formatter == "money") {
			if (value == "" || value == "null" || value == undefined) {
	...
	getEditorFormValue: function (form, dataSet, value, key, CH, idAttr) {
		...
		if (form.type == "hidden") {
			...
			var formValue = getFormValue(form.value, dataSet[key]);
			$.each(form.options, function (oidx, opt) {
					...
					if (isfix == undefined || (isfix != undefined && colCount < (cfg.fixedColSeq + 1))) {
						...
						if ((hasFixed && !CH.isFixedCell) || !hasFixed || isfix != undefined) {
							if (CH.form) {
								...
								tpo.push(getEditorFormatterValue(CH.formatter, dataSet, dataSet[CH.key], CH.key, CH));
							} else {
								tpo.push(dataSet[CH.key]);
							}
						} else {
	...
	setEditor: function (item, itemIndex, insertIndex) {
		...
		if (itemIndex != undefined) {
			...
			var scrollTop = $("#" + cfg.targetID + "_AX_scrollContent").position().top;
			var editorTop = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + itemIndex).position().top;
			var list = this.list;
			var itemTRheight = (function () {
				if (list.length == 0) {
					return 0;
				} else if (list.length == 1) {
					var p2 = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + itemIndex).height();
					return p2;
				} else if ((list.length - 1) == itemIndex) {
					var p1 = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + (itemIndex - 1)).position().top;
					var p2 = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + itemIndex).position().top;
					return p2 - p1;
				} else {
					var p1 = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + (itemIndex)).position().top;
					var p2 = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + (itemIndex.number() + 1)).position().top;
					return p2 - p1;
				}
			})();
			...
			var scrollTop = $("#" + cfg.targetID + "_AX_scrollContent").position().top;
			var editorTop = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + insertIndex).position().top;
			var list = this.list;
			var itemTRheight = (function () {
				if (list.length == 0) {
					return 0;
				} else if (list.length == 1) {
					var p2 = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + insertIndex).height();
					return p2;
				} else if ((list.length - 1) == itemIndex) {
					var p1 = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + (insertIndex - 1)).position().top;
					var p2 = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + insertIndex).position().top;
					return p2 - p1;
				} else {
					var p1 = $("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + (insertIndex)).position().top;
					var p2 = ($("#" + cfg.targetID + "_AX_tr_0_AX_n_AX_" + (insertIndex.number() + 1)).position() || 0).top;
					return p2 - p1;
				}
			})();
			this.editor.css({ top: editorTop + scrollTop });
			...
			$("#" + cfg.targetID + "_AX_scrollContent").css({ top: trTop });
			...
			$("#" + cfg.targetID + "_AX_scrollContent").css({ top: 0 });
			this.contentScrollContentSync({ top: 0 });

		}

		$("#" + cfg.targetID + "_AX_scrollTrackXY").before(this.editor);
		this.editor.show();
						...
						if (CH.form.onChange) {
							$("#" + formID).bind("change", function () {
								CH.form.onChange.call({
									...
									position: CHidx,
									value: $("#" + formID).val(),
									text: $M(formID).options[$M(formID).options.selectedIndex].text
								});
							});
						}
						if (CH.form.onClick) {
		...
		var editorContentHeight = $("#" + cfg.targetID + "_AX_editorContent").height();
		var fixedEditorContentHeight = $("#" + cfg.targetID + "_AX_fixedEditorContent").height();
		if (editorContentHeight < fixedEditorContentHeight) {
			editorContentHeight = fixedEditorContentHeight;
			$("#" + cfg.targetID + "_AX_editorContent").find(".gridBodyTable").css({ height: editorContentHeight });
		} else {
			$("#" + cfg.targetID + "_AX_fixedEditorContent").find(".gridFixedBodyTable").css({ height: editorContentHeight });
		}
		$("#" + cfg.targetID + "_AX_editorButtons").css({ top: editorContentHeight });
		...
		if (editorPosHeight > scrollHeight) {
			$("#" + cfg.targetID + "_AX_scrollContent").append("<div style=\"height:" + (editorPosHeight - scrollHeight) + "px;\"></div>");
			var scrollContentTop = $("#" + cfg.targetID + "_AX_scrollContent").position().top - (editorPosHeight - scrollHeight);
			this.contentScrollYAttr = null;
			this.contentScrollResize(false);
		}

		var scrollLeft = $("#" + cfg.targetID + "_AX_scrollContent").position().left;
		$("#" + cfg.targetID + "_AX_editorContent").css({ left: scrollLeft });

		$("#" + cfg.targetID + "_AX_editorButtons_AX_save").bind("click", this.saveEditor.bind(this));
		$("#" + cfg.targetID + "_AX_editorButtons_AX_cancel").bind("click", this.cancelEditor.bind(this));
	},
	setEditorForm: function (obj) {
		var cfg = this.config;
		var formID = cfg.targetID + "_AX_" + obj.key + "_AX_" + obj.position.join("_AX_");
		if (!$M(formID)) alert(formID + "로 Element를 찾을 수 없습니다.");
		$("#" + formID).val(obj.value);
	},
	saveEditor: function () {
		var cfg = this.config;
			...
			$.each(cfg.editor.rows[r], function (CHidx, CH) {
				if (CH.form) {
					...
					if (CH.form.type == "radio"){
						var checkedValue = [];
						$.each(CH.form.options, function (oidx, opt) {
							var opt_formID = formID + "_AX_" + oidx;
							if ($("#" + opt_formID).get(0).checked) editorFormItem.push(CH.key + "=" + $("#" + opt_formID).val());
						});
						...
						if (CH.form.value == "itemText") {
							editorFormItem.push(CH.key + "=" + $M(formID).options[$M(formID).options.selectedIndex].text);
						} else {
							editorFormItem.push(CH.key + "=" + $("#" + formID).val());
						}
					} else {
						editorFormItem.push(CH.key + "=" + $("#" + formID).val());
					}
		...
		if (cfg.editor.request) {
			var validate = function (formID, CH) {
				...
				if (CH.form.type == "radio") {
					$.each(CH.form.options, function (oidx, opt) {
						var opt_formID = formID + "_AX_" + oidx;
						if ($("#" + opt_formID).get(0).checked) checkedValues.push($("#" + opt_formID).val());
					});
					...
					if (CH.form.value == "itemText") {
						value = $M(formID).options[$M(formID).options.selectedIndex].text;
					} else {
						value = $("#" + formID).val();
					}
				} else {
					value = $("#" + formID).val().trim();
				}
				var sendObj = {
					formID: formID,
					value: value,
	...
	setPaging: function () {
		var cfg = this.config;
		// apply page vars
		var pageNos = $M(cfg.targetID + "_AX_gridPageNo");
		...
		if (pgCount == 0) {
			...
			$("#" + cfg.targetID + "_AX_gridPageNo").html(po.join(''));
		} else {
			var mySelect = $M(cfg.targetID + "_AX_gridPageNo");
			if (pgCount > 1000) {
				var oi = 0;
				var pageStart = 1;
			...
			//
			//alert($M(cfg.targetID + "_AX_gridPageNo").options[$M(cfg.targetID + "_AX_gridPageNo").options.selectedIndex].value);
		}
		$("#" + cfg.targetID + "_AX_gridPageCount").html("/ " + pgCount.money() + " Pages");
		$("#" + cfg.targetID + "_AX_gridStatus").html("전체 <b>" + this.page.listCount.number().money() + "</b>개의 목록이 있습니다.");

		if (this.isMobile) {
			$("#" + cfg.targetID + "_AX_gridPageNo").bind("change", this.onPageChange.bind(this));
		} else {
			var onPageChange = this.onPageChange.bind(this);
			$("#" + cfg.targetID + "_AX_gridPageNo").bindSelect({
	...
	goPageMove: function (pageAdd) {
		...
		if (this.isMobile) {
			$("#" + cfg.targetID + "_AX_gridPageNo").val(pageNo);
			this.onPageChange();
		} else {
			$("#" + cfg.targetID + "_AX_gridPageNo").setValueSelect(pageNo);
		}
	...
	onPageChange: function () {
		...
		var pgCount = this.page.pageCount.number();
		var pageNo = this.page.pageNo.number();
		var npageNo = $("#" + cfg.targetID + "_AX_gridPageNo").val();
	...
	setStatus: function (listLength) {
		var cfg = this.config;
		$("#" + cfg.targetID + "_AX_gridStatus").html("전체 <b>" + listLength.number().money() + "</b>개의 목록이 있습니다.");
	},
	getSortParam: function (ty) {
		...
		if (sortObj) {
			if (ty == "one") {
				...
				return $.param({ sortKey: sortObj.key, sortWay: sortObj.sort });
			}
		} else {
			return "";
		}
