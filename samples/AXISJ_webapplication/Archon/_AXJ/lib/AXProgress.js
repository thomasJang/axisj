...
var AXProgress = Class.create(AXJ, {
	...
	start: function(callBack, options){
		...
		po.push("</div>");
		this.progress = $(po.join(''));
		$(document.body).append(this.progress);
		
		$("#"+progressID+"_AX_cancel").bind("click", this.cancel.bind(this));
	...
	update: function(){
		...
		$("#"+progressID+"_AX_loadedText").html(loadedRate+"%<span>"+loadedCount.money()+"/"+totalCount.money()+"</span>");
		
		if(theme == "AXlineProgress"){
			$("#"+progressID+"_AX_bar").animate(
				{width:loadedRate+"%"},
				config.duration, "", 
				function(){
					if(config.callBack){
			...
			setTimeout(function(){
				$("#"+progressID+"_AX_bar").addClass("percent"+((loadedCount / totalCount * 100).round(0) / 5).round() * 5);
				if(config.callBack){
					config.callBack.call({
						totalCount:totalCount,
						loadedCount:loadedCount,
