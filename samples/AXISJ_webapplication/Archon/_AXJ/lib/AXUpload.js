...
SWFUpload.prototype.loadFlash = function () {
	...
	$(targetElement).empty();
	$(targetElement).html(this.getFlashHTML());
...
var AXUpload = Class.create(AXJ, {
	...
	init: function(){
		...
		if($M(this.settings.queueBox_id)){
			this.isMultiUpload = true;
			$("#"+this.settings.queueBox_id).html("<div style=\"clear:both;height:20px;\"></div>");
	...
	fileQueued: function(file){
		...
		if(this.isMultiUpload){
			...
			po.push("</div>");
			$("#"+this.settings.queueBox_id).prepend(po.join(''));
			$("#"+file.id).show("slow");
			$("#"+file.id).find(".AXUploadBtnsA").bind("click", this.onClickButton.bind(this));
	...
	uploadProgress: function(file, bytesLoaded, bytesTotal){
		if(this.isMultiUpload){
			//진행중
			//alert("3. 진행중");
			$("#"+file.id).find(".AXUploadProcessBar").width( ((bytesLoaded/bytesTotal)*100).round()+"%" );
		}else{
			//
			var UploadDisplay_id = this.settings.UploadDisplay_id;
			//trace("single - uploadProgress : " + ((bytesLoaded/bytesTotal)*100).round()+"%");	
			$("#"+UploadDisplay_id).find(".AXUploadProcessBar").width( ((bytesLoaded/bytesTotal)*100).round()+"%" );
		}
	},
	uploadSuccess: function(file, serverData){
		...
		if(this.isMultiUpload){
			...
			$("#"+file.id).find(".AXUploadBtns").fadeIn();
			$("#"+file.id).find(".AXUploadProcess").fadeOut();
			...
			if(SD.thumbName != ""){
				$("#"+file.id).find(".AXUploadIcon").get(0).style.backgroundImage="url('"+(AXfile.thumb||"").dec()+"')";
				$("#"+file.id).find(".AXUploadIcon").get(0).style.backgroundSize="100% auto";
				$("#"+file.id).find(".AXUploadIcon").get(0).style.backgroundPosition="center center";
				//$("#"+file.id).find(".AXUploadIcon").get(0).style.backgroundImage="url('/_FILE/2013_06/thumb/thumb_20130621152635001.jpg')";
			}
			
			if(this.settings.onclick){
				var onclick = this.settings.onclick.bind(this);
				$("#"+file.id).find(".AXUploadIcon").bind("click", function(){
					onclick(AXfile);
				});
			}
			
			$("#"+file.id).addClass("readyselect");			
			...
			$("#"+UploadDisplay_id).find(".AXUploadProcess").fadeOut(function(){
	...
	onClickButton: function(event){
		...
		if(this.isMultiUpload){
			while(!$(myTarget).hasClass("AXUploadBtnsA")){
			...
			if(!$(myTarget).hasClass("AXFileDelete")){
				return;	
			}
	...
	deleteFile: function(file, onEnd){
		...
		if (file != undefined){
				...
				if(this.isMultiUpload){
					...
					$("#"+file.id).find(".AXUploadBtns").fadeOut();
					...
					var UploadDisplay_id = this.settings.UploadDisplay_id;
					var onClickButton = this.onClickButton.bind(this);
					$("#"+UploadDisplay_id).html("<div class=\"AXLoadingSmall\" style=\"width:25px;\"></div>");
					...
					new AXReq(url, {debug:false, pars:pars, onsucc:function(res){
						if(res.result == AXUtil.ajaxOkCode){
							...
							$("#"+UploadDisplay_id).html("업로드 하실 파일을 선택해 주세요");
	...
	setUploadedList: function(files){
		...
		$.each(this.uploadedList, function(uidx, U){
		...
		$("#"+this.settings.queueBox_id).prepend(po.join(''));
		$("#"+this.settings.queueBox_id).find(".AXUploadBtnsA").bind("click", this.onClickButton.bind(this));
		
		if(this.settings.onclick){
			var onclick = this.settings.onclick.bind(this);
			var _uploadedList = this.uploadedList;
			$("#"+this.settings.queueBox_id + " .AXUploadItem").find(".AXUploadIcon").bind("click", function(event){
	...
	setLabelImageFile: function(argObj){
		...
		$(".AXUploadLabel").hide();
		$("#mainImage").val(argObj.fileName);
		$("#"+fileID+"_AXUploadLabel_labelImageFile").parent().show();
	},
...
var __AXUpload = {
	...
	bind: function(targetID, setting){
		...
		$.each(__AXUpload.instances, function(iidx, I){
	...
	setUploadedFile: function(targetID, file){
		...
		var insSeq = null;
		$.each(__AXUpload.instances, function(idx, Obj){
			if(this.targetID == targetID){
				insSeq = idx;
			}
		});
