...
var AXTopDownMenu = Class.create(AXJ, {
	...
	onoverParent: function(event){
		...
		if(this.poi != "" && this.poi != poi){
			$("#" + cfg.menuBoxID + "_PMA_" + this.poi).removeClass("on");
			$("#" + cfg.menuBoxID + "_PMC_" + this.poi).slideUp(
				{
		...
		//slideDown check
		if(this.dfPoi != undefined) $("#" + cfg.menuBoxID + "_PMA_" + this.dfPoi).removeClass("on");
		$("#" + cfg.menuBoxID + "_PMA_" + poi).addClass("on");
		//trace("#" + cfg.menuBoxID + "_PMC_" + poi);
		
		var tgDiv = $("#" + cfg.menuBoxID + "_PMC_" + poi);
		...
		if(item){
			if(item.id){
				...
				if(!item.divDim){
					...
					var pDim = {width:$("#"+eid).outerWidth(), height:$("#"+eid).outerHeight(), pos:$("#"+eid).position()};
	...
	outChild: function(){
		...
		$("#" + cfg.menuBoxID + "_PMA_" + this.poi).removeClass("on");
		if(this.dfPoi != undefined) $("#" + cfg.menuBoxID + "_PMA_" + this.dfPoi).addClass("on");
		$("#" + cfg.menuBoxID + "_PMC_" + this.poi).slideUp(
			{
				duration:cfg.easing.close.duration,
				easing:cfg.easing.close.easing,
	...
	setHighLightMenu: function(poi){
		...
		if($.isArray(poi)){
			...
			var tree = this.tree;
			$.each(poi, function(idx, T){
				if(idx == 0) tree = tree[T.number()];
				else  tree = tree.cn[T.number()];
				if(tree){
					if(idx == 0){
						$("#" + tree.id).addClass("on");
						$("#" + tree.id).children("A").addClass("on");
					}else{
						$("#" + tree.id.replace("_PMC_", "_PMA_")).addClass("on");
					}
				}
			});
			...
			this.dfPoi = poi;
			$("#" + cfg.menuBoxID + "_PMA_" + this.dfPoi).addClass("on");
