...
var AXEditor = Class.create(AXJ, {
	...
	closePanel: function(){
		...
		$("#"+config.targetID+"_head").removeClass("editorHeadExt");	
	},
	openPanel: function(){
		var config = this.config;
		var re = config.targetID;
		...
		$("#"+config.targetID+"_head").addClass("editorHeadExt");
		if(this.panelNum == 1){
		...
		if(this.panelNum == 5){
			$("#"+re+"_divPanel_external").find(".componentItem").bind("click", this.onClickComponent.bind(this));
		}
	},
	...
	readyFrame: function(){
		...
		try{
			$(editorBody.body).css({"font-size":config.editorFontSize});
			$(editorBody.body).css({"font-family":config.editorFontFamily});
		    //if(this.config.readyTofocus) editorBody.body.focus();
		}catch(e){
		}
	...
	unbindFocusFrame: function(){
		var editorBody = this.myEDT.document;
		if(AXUtil.browser.name == "mozilla") $(editorBody).unbind("focus", this.focusFrame.bind(this));
	    else	$(editorBody.body).unbind("focus", this.focusFrame.bind(this));
	},
	bindFocusFrame: function(){
		var editorBody = this.myEDT.document;
		if(AXUtil.browser.name == "mozilla") $(editorBody).bind("focus", this.focusFrame.bind(this));
	...
	__pasteHTML: function(sHTML){
		...
		if(this.contentMode == "editor"){
			...
			$("#"+this.config.targetID+"_html").val($("#"+this.config.targetID+"_html").val()+oTmpDiv.innerHTML);
		}
	},
	/* click event */
	...
	onFeilDeleteInFrame: function(rid){
		...
		if(AXUtil.browser.name == "mozilla"){
			$(editorBody).find(rid).remove();
		}else{
			$(editorBody.body).find(rid).remove();
		}
	},
	...
	onClickComponent: function(event){
		...
		if(!myID){
			try{
				myID = $(event.target).parents(".componentItem")[0].id;
			}catch(e){
			}
			if(!myID) return;
		};
	...
	icoPicker: function(command){
		...
		$.each(AXEditorIcons, function(idx, ca){
		...
		$.each(AXEditorIcons[icoCategoryIndex].icons, function(index, n){
	...
	changeCategory: function(event){
		...
		$("#"+re+"_icoBot").html(AXEditorIcons[icoCategoryIndex].copyRights);
	},
	/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
	onContentMode: function(event){
		...
		if(event.target.id == config.targetID+"_contentMode_0"){
			if(this.contentMode != "editor"){
				this.contentMode = "editor";
				$("#"+config.targetID+"_contentMode_0").addClass("me____tabOn");
				$("#"+config.targetID+"_contentMode_1").removeClass("me____tabOn");
				this.setHtmlMode("off");
			}
		}else{
			if(this.contentMode != "html"){
				this.contentMode = "html";
				$("#"+config.targetID+"_contentMode_0").removeClass("me____tabOn");
				$("#"+config.targetID+"_contentMode_1").addClass("me____tabOn");
				this.setHtmlMode("on");
			}
		}
		//goto home
	...
	setContent: function(content){
		...
		if(typeof content == "string"){
			...
			if(myBody.html() == null){
				$(myEDT.body).html(content.html());
			}else{
				$(myEDT.body).html(myBody.html());
				$(myEDT.body).css({"font-family":myBody.css("fontFamily")});
				$(myEDT.body).css({"font-size":myBody.css("fontSize")});
				$(myEDT.body).css({"color":myBody.css("color")});
				$(myEDT.body).css({"background-color":myBody.css("backgroundColor")});
				$(myEDT.body).css({"line-height":myBody.css("lineHeight")});
			}
		}
	},
